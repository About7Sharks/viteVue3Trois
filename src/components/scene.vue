<template>
  <Renderer ref="renderer" antialias  resize="window" :orbit-ctrl="{ enableDamping: true, dampingFactor: 0.05 }" pointer shadow>
    ...
    <Scene>
      ...
      <InstancedMesh ref="imesh" :count="NUM_INSTANCES" cast-shadow receive-shadow>
        ...
        <PhongMaterial color="yellow" />
      </InstancedMesh>

      <Plane :width="W*2" :height="H*2" :position="{ z: -30 - SIZE }" receive-shadow>
        <PhongMaterial color="purple" />
      </Plane>
    </Scene>
  </Renderer>
</template>
...
export default {
  ...
  setup() {
    const SIZE = 2, NX = 20, NY = 20, PADDING = 1;
